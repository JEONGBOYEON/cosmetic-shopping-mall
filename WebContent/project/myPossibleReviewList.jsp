...
<div class="ap_contents mypage coupon">
	...
	<div class="ui_tab">
		...
		<div class="tab_menu equally">
			<ul>
				<li><a href="<%=cp%>/review/list.do">작성한 리뷰 <b id="availCnt">${dataCount_yes }</b>개</a></li>
				<li class="on"><a href="<%=cp%>/review/possibleList.do">작성 가능한 리뷰 <b id="expCnt">${dataCount_no }</b>개</a></li>
			</ul>
		</div>
					...
					<tbody id="paging">
						<c:forEach var="dto" items="${lists }">
							<tr>
								...
								<td><a href="<%=cp%>/pr/detail.do?productName=${dto.productName1}">${dto.productName2 }</a></td>
								<td>
									<button onclick="javascript:location.href='<%=cp%>/review/writed.do?productId=${dto.productId }';" class="btn_sm_bordered">작성하기</button>
								</td>
							</tr>
						</c:forEach>
					</tbody>
